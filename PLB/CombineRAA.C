                ...
                Bool_t verbose=true)
{
  ...
  Float_t xaxismin = isHadDupl==1?0.7:(isAlice==1?0.6:1.0);
  ...
  if(isNjpsi==3 && centmin==0 && centmax==100)
    {
      canvasRAAPbPb_0_100_BtoDRAA();
    }
  ...
  TLatex* texcms = (isTheoryComparison==0&&isAlice==0&&((isHadDupl==0&&isB==0&&isNjpsi==0)||(isHadDupl==1&&isB==1&&(isNjpsi==0))))?texcmssupp:texcmspaper;
  texcms->Draw();
  ...
  if((isHadDupl==1 && isB==0 &&isNjpsi==0)) pindx = 1;
  ...
  if((isHadDupl==1 && isB==1 && isNjpsi==0) || (isAlice==1 && isHadDupl==0 && isB==0 && isNjpsi==0)) pindx = 5;
  ...
  if(isNjpsi==3)
    {
      TLegend* legBtoDRAA = new TLegend(0.23, 0.76, 0.68, 0.81);
      setleg(legBtoDRAA, legtexsize[pindx]);
      legBtoDRAA->AddEntry(grae2, "b #rightarrow D^{0}", "pf");
      legBtoDRAA->Draw();
    }

  if(isAlice==1) 
    {
      legendRAA->AddEntry(grRAAalicesyst, "ALICE |y| < 0.5","pf");
      legendRAA->AddEntry(grRAAstarsyst, "STAR 2014","pf");
    }
  ...
  TString texNjpsi = (isNjpsi>0)?(isNjpsi==1?"_Njpsi2TeV":(isNjpsi==2?"_Njpsi5TeV":"_Njpsi5TeV_BtoD")):"";
  ...
  std::cout<<"Global uncertainties: low = "<<systnormlo*1.e+2<<"%, up = "<<systnormup*1.e+2<<"%"<<std::endl;
}
