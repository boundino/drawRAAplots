...
void drawStarD0Raa()
{
  ...
  std::ifstream getdata("StarRaa/StarD0Raa_0_10_new2014.dat"); 
  for(int i=0;i<BIN_NUM;i++)
    {
      Float_t tem;
      getdata>>apt[i]>>acentral[i]>>tem>>asystl[i]>>tem>>asysth[i];
      //aptl[i] = 0;
      //apth[i] = 0;
      ...
      std::cout<<apt[i]<<" "<<acentral[i]<<" "<<asysth[i]<<" "<<asystl[i]<<std::endl;
    }
  ...
  grRAAstarsyst->SetMarkerColor(kBlack);
  grRAAstarsyst->SetMarkerSize(1.2);
  grRAAstarsyst->SetLineColor(kBlack);
  grRAAstarsyst->SetLineWidth(2);
  grRAAstarsyst->SetFillStyle(0);

  // grRAAstarsyst->Draw("[]p same");
  grRAAstarsyst->Draw("5p same");

}

void drawStarD0Raa_old()
{
  const int BIN_NUM = 7;
  Float_t apt[BIN_NUM], aptl[BIN_NUM], apth[BIN_NUM], acentral[BIN_NUM], astat[BIN_NUM], asystl[BIN_NUM], asysth[BIN_NUM], azero[BIN_NUM], aptlnarrow[BIN_NUM], apthnarrow[BIN_NUM];
